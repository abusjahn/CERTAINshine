...
ui <- fluidPage(
  dashboardPage(
    ...
    body = dashboardBody(
      ...
      fluidRow(
        # column(1,
        #        icon(class="duotone", name="file-excel")),
        column(12,
               textOutput('filename'),
               tags$head(tags$style("#filename{color: blue;
                                            font-size: 16px;
                       font-style: bold;
                       }")),
                 ...
                 tabsetPanel(
                   ...
                   tabPanel('Import/Export',
                            fluidRow(
                              ...
                              column(
                                width=2,
                                checkboxInput(inputId = 'dropnopat',label = 'Drop non-patient sheets?',
                                              value = TRUE)
                              )),
  ...
  server <- function(input, output) {
    ...
    hl_file <- reactive({
      # req(ct_datalist())
      paste('File selected:',input$file$name)
    })
